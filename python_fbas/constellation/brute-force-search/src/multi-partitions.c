...
// But note that v[i] = 0 may appear for some is

// Function to print part k of a partition:
void print_part(int c[], int v[], int f[], int k) {
    printf("%d", c[f[k]]);
    for (int n = 1; n < v[f[k]]; n++) {
        printf(".");
        printf("%d", c[f[k]]);
    }
    for (int i = f[k]+1; i < f[k+1]; i++) {
        for (int n = 0; n < v[i]; n++) {
            printf(".");
            printf("%d", c[i]);
        }
...
void generate_partitions(int n[], size_t m, visit_func_t visit, size_t min_size, size_t max_parts) {
    ...
    if (k > b) {
        ...
        l = l + 1;
        f[l+1] = b;
        // if u is too small or l is too large, backtrack
        if (part_size(c, u, f, l) < min_size || l >= max_parts) {
           goto m6;
        }
        goto m2;
    }
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        fprintf(stderr, "Usage: %s <min_size> <max_parts> <n1> <n2> ... <nm>\n", argv[0]);
        return 1;
    }
    int min_size = atoi(argv[1]);
    int max_parts = atoi(argv[2]);
    ...
    generate_partitions(n, m, example_visit_partition, min_size, max_parts);
